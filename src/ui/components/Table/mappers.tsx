...
export function renderFlag(flag?: boolean): ReactElement {
  return pipe(
    ...
    Option.fold(
      () => (<></>),
      (someFlag) => (
        <Tag color={someFlag ? 'green' : 'volcano'}>
          {someFlag ? 'Yes' : 'No'}
        </Tag>
      ),
    ),
...
export function renderTagsWithClickHandler(
...
export function renderActionsAsColumn<Item>(
  columnWidth: number,
) {
  return function getActionsColumn(
    getActions: (record: Item) => ReactElement,
  ) {
    return {
